...
import cn.thinking.common.util.ReflectHelper;
import cn.thinking.common.util.Tools;

/**
 * 
 * Description: 结合page分页类,对数据库查询分页的支持组件
 * @update [修改人] [修改时间]
 * @version 1.0
 *
 */
...
//                            ((cn.thinking.org.system.trainer.Trainer)parameterObject).getPage().getTotalPage()
                        }
                        else {
                    ...
                    System.err.println("执行分页的SQL语句为：" + pageSql);
                }
            }
        }
        return ivk.proceed();
