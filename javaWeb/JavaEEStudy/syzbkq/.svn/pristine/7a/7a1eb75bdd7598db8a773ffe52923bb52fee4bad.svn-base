(function ($) {
    ...
    MultipleSelect.prototype = {
        ...
        events: function () {
            ...
            //修复下拉列表点击页面其余位置不能收回的BUG
            //add by changcheng
            this.$choice.off('blur').on('blur',  function(e){
            	$('body').click(function (e) {
                    if ($(e.target)[0].offsetParent.className == "ms-drop bottom" || $(e.target)[0].offsetParent.className == "ms-parent") {
                    	return;
                    }else{
                    	that.close();
                    }
